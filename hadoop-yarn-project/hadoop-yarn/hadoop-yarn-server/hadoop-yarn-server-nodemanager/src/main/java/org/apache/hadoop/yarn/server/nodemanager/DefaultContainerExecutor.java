...
public class DefaultContainerExecutor extends ContainerExecutor {
    ...
    public int launchContainer(Container container,
        ...
        //生成磁盘日志目录 详见参考/data/yarn/container/log 等
        ...
        // Create new local launch wrapper script 本地启动脚本 可以认为就是java -jar app.jar模式
        ...
        // Windows path length limitation. 判断是否在window目前hadoop并不支持window，默认都是以linux为准
        if (Shell.WINDOWS &&
        ...
        // fork script  每个容器下载的jar，通过shell脚本进行执行，其实就是java -jar app.jar而真正container 其实就是虚拟隔离
